...
#import "KFSupportCategoryViewController.h"
#import "KFSupportArticleViewController.h"
...
@interface KFSupportApiViewController ()<QMUISearchControllerDelegate>
...
@implementation KFSupportApiViewController
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        BDCategoryModel *categoryModel = [self.mCategoryArray objectAtIndex:indexPath.row];
        //
        KFSupportCategoryViewController *categoryVC = [[KFSupportCategoryViewController alloc] initWithStyle:UITableViewStyleGrouped];
        [categoryVC initWithCategoryModel:categoryModel];
        [self.navigationController pushViewController:categoryVC animated:YES];
    } else {
        BDArticleModel *articleModel = [self.mArticleArray objectAtIndex:indexPath.row];
        //
        KFSupportArticleViewController *articleVC = [[KFSupportArticleViewController alloc] initWithStyle:UITableViewStyleGrouped];
...
- (void)searchController:(QMUISearchController *)searchController updateResultsForSearchString:(NSString *)searchString {
    ...
    //
    [BDCoreApis searchArticle:DEFAULT_TEST_ADMIN_UID withContent:searchString resultSuccess:^(NSDictionary *dict) {
...
- (void)getCategories {
    //
    [BDCoreApis getSupportCategories:DEFAULT_TEST_ADMIN_UID resultSuccess:^(NSDictionary *dict) {
//        DDLogInfo(@"%s, %@", __PRETTY_FUNCTION__, dict);
        //
        NSNumber *status_code = [dict objectForKey:@"status_code"];
        if ([status_code isEqualToNumber:[NSNumber numberWithInt:200]]) {
...
- (void)getArticles {
    //
    [BDCoreApis getSupportArticles:DEFAULT_TEST_ADMIN_UID resultSuccess:^(NSDictionary *dict) {
        DDLogInfo(@"%s, %@", __PRETTY_FUNCTION__, dict);
        //
        NSNumber *status_code = [dict objectForKey:@"status_code"];
        if ([status_code isEqualToNumber:[NSNumber numberWithInt:200]]) {
