...
@interface SWQRCodeViewController ()<AVCaptureMetadataOutputObjectsDelegate, AVCaptureVideoDataOutputSampleBufferDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, SFSafariViewControllerDelegate>
...
- (void)sw_handleWithValue:(NSString *)value {
    ...
    if ([value hasPrefix:@"http"]) {
        //
        [self handleUrlResult:value];
        // 关闭当前vc
        [self.navigationController popViewControllerAnimated:YES];
    } else {
        DDLogInfo(@"非网址");
        [QMUITips showInfo:value inView:self.view];
    }
}
...
- (void)sw_didReadFromAlbumFailed {
    ...
    [QMUITips showError:@"读取相册失败" inView:self.view hideAfterDelay:2.0f];
}
...
- (void)handleUrlResult:(NSString *)urlString {
