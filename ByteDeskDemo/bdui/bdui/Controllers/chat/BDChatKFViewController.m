...
- (void)chooseWorkGroupLiuXue:(NSString *)workGroupWid withWorkGroupNickname:(NSString *)nickname {
    ...
    [BDCoreApis requestChooseWorkGroupLiuXueLBS:workGroupWid withWorkGroupNickname:nickname withProvince:@"辽宁" withCity:@"大连" resultSuccess:^(NSDictionary *dict) {
        ...
        NSString *newWid = dict[@"data"][@"wid"];
        
        self.mWorkGroupWid = newWid;
...
//            [self reloadCellDataStatus:localId];
//
//        } else {
//            // 修改本地消息发送状态为error
...
- (void)uploadImageData:(NSData *)imageData {
    ...
    [BDCoreApis uploadImageData:imageData withImageName:imageName resultSuccess:^(NSDictionary *dict) {
        ...
        if ([status_code isEqualToNumber:[NSNumber numberWithInt:200]]) {
            ...
            // 异步发送图片消息
            //         [[BDMQTTApis sharedInstance] sendImageMessage:imageUrl toTid:self.mTidOrUidOrGid localId:localId sessionType:self.mThreadType];
            ...
            [BDCoreApis sendImageMessage:imageUrl toTid:self.mTidOrUidOrGid localId:localId sessionType:self.mThreadType resultSuccess:^(NSDictionary *dict) {
                ...
                if ([status_code isEqualToNumber:[NSNumber numberWithInt:200]]) {
                    ...
                    // 修改本地消息发送状态为成功
                    [[BDDBApis sharedInstance] updateMessageSuccess:localId];
                    [self reloadCellDataStatus:localId status:BD_MESSAGE_STATUS_STORED];
...
- (void) robotLinkClicked:(NSString *)label withKey:(NSString *)key; {
    DDLogInfo(@"%s label:%@, key:%@", __PRETTY_FUNCTION__, label, key);
    ...
    if ([key hasPrefix:@"helpfull"]) {
        ...
        DDLogInfo(@"有帮助 %@, aid: %@, mid: %@", [array objectAtIndex:0], aid, mid);
        //
        ...
        NSString *aid = [array objectAtIndex:1];
        NSString *mid = [array objectAtIndex:2];
        DDLogInfo(@"无帮助 %@, aid: %@, mid: %@", [array objectAtIndex:0], aid, mid);
        //
        ...
        NSArray *array = [key componentsSeparatedByString:@":"];
        NSString *aid = [array objectAtIndex:1];
        DDLogInfo(@"查询问题答案 %@, aid: %@", [array objectAtIndex:0], aid);
        // 此处key即为aid
...
- (void)notifyMessageRecall:(NSNotification *)notification {
    ...
    DDLogInfo(@"%s mid: %@", __PRETTY_FUNCTION__, mid);
    //
    for (int i = 0; i < [self.mMessageArray count]; i++) {
        BDMessageModel *message = [self.mMessageArray objectAtIndex:i];
