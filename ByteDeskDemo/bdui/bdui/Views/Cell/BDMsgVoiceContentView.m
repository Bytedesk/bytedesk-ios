...
- (void)handleVoiceClicked:(id *)sender {
    DDLogInfo(@"%s, %@", __PRETTY_FUNCTION__, self.model.voice_url);
    ...
    if (error) {
        DDLogError(@"Init AudioSession Error: domain:%@ code:%ld description:%@", [error domain], (long)[error code], [[error userInfo] description]);
        return;
    }
    ...
    if (error) {
        DDLogError(@"AudioSession setActive Error: %@ %ld %@", [error domain], (long)[error code], [[error userInfo] description]);
        return;
    }
...
//                        [KFUtils amrToWav:amrSavePath wavSavePath:voicePath];
        ...
        if ([[NSFileManager defaultManager] fileExistsAtPath:voicePath]) {
            ...
            if (error) {
                DDLogError(@"AVAudioPlayer init Error: domain:%@ code:%ld userInfo:%@", [error domain], (long)[error code], [error userInfo]);
                return;
            }
        }
        else {
